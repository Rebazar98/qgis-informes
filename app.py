...
# Rutas/nombres por defecto (puedes sobreescribir con vars de entorno en Railway)
QGIS_PROJECT = os.getenv("QGIS_PROJECT", "/app/proyecto.qgz")
QGIS_LAYOUT  = os.getenv("QGIS_LAYOUT",  "Plano_urbanistico_parcela")
QGIS_ALGO    = os.getenv("QGIS_ALGO",    "native:printlayouttopdf")  # este sabemos que existe
...
def render(
    ...
    # COMANDO CORRECTO:
    # qgis_process --project-path /app/proyecto.qgz run native:printlayouttopdf -- LAYOUT=... OUTPUT=...
    cmd: List[str] = [
        "xvfb-run",
        "-a",
        "qgis_process",
        "--project-path",
        QGIS_PROJECT,         # opciones globales ANTES de 'run'
        "run",
        QGIS_ALGO,            # native:printlayouttopdf
        "--",                 # a partir de aquí, parámetros del algoritmo
    ...
    # Variable de entorno para el filtro del Atlas: env('REFCAT')
    extra_env = {
        "REFCAT": refcat,
    }
